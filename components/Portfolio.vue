<template>
  <div>
    <v-row class="tw-mx-8">

      <v-col class="" cols="12" md="4" lg="4">

        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText = true" @mouseleave="showHoverText = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-1.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://sylviora.com/" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>


        </div>



      </v-col>

      <v-col class="" cols="12" md="4" lg="4">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText2 = true" @mouseleave="showHoverText2 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-2.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText2"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://msdemo.ova-africa.com/" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>
      </v-col>

      <v-col class="" cols="12" md="4" lg="4">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText3 = true" @mouseleave="showHoverText3 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-3.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText3"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://mekengroup.com/" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>


      </v-col>

      <v-col class="" cols="12" md="4" lg="4">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText4 = true" @mouseleave="showHoverText4 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-4.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText4"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://kenayafinance.ova-africa.com/" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>


      </v-col>

      <v-col class="" cols="12" md="4" lg="4">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText5 = true" @mouseleave="showHoverText5 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-5.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText5"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://route360solutionsinitiative.com/" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>
      </v-col>

      <!-- <v-col class="" cols="12" md="4" lg="4">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText6 = true" @mouseleave="showHoverText6 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-6.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText6"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="http://" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>

      </v-col> -->

      <v-col class="" cols="12" md="4" lg="4">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText7 = true" @mouseleave="showHoverText7 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-8.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText7"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://e-tuto.jumbasoft.com/" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>

      </v-col>

      <v-col class="" cols="12" md="4" lg="4" v-if="showMore">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText8 = true" @mouseleave="showHoverText8 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-7.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText8"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://shebaplastic.com/" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>

      </v-col>

      <v-col class="" cols="12" md="4" lg="4" v-if="showMore">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText9 = true" @mouseleave="showHoverText9 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-9.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText9"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://github.com/ALUtech166/spotify-clone.git" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>

      </v-col>

      <v-col class="" cols="12" md="4" lg="4" v-if="showMore">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText10 = true" @mouseleave="showHoverText10 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-10.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText10"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://github.com/ALUtech166/whatsapp-web-clone.git" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>

      </v-col>

      <v-col class="" cols="12" md="4" lg="4" v-if="showMore">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText11 = true" @mouseleave="showHoverText11 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-11.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText11"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://manglam.mekengroup.com/" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>

      </v-col>
      <v-col class="" cols="12" md="4" lg="4" v-if="showMore">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText13 = true" @mouseleave="showHoverText13 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText13"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://github.com/ALUtech166/netflix-clone.git" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>

      </v-col>
      <v-col class="" cols="12" md="4" lg="4" v-if="showMore">
        <div class="tw-cursor-pointer tw-bg-white tw-block tw-w-full tw-shadow-xl tw-rounded-lg tw-shadow-gray-100">
          <div @mouseenter="showHoverText12 = true" @mouseleave="showHoverText12 = false"
            class="tw-relative tw-w-full tw-h-72 hover:tw-shadow-yellow-700 hover:tw-shadow-xl hover:tw-ease-out  hover:tw-transition-all hover:tw-rounded">
            <img class="tw-absolute tw-top-0 tw-left-0 tw-rounded-lg tw-w-full tw-h-full tw-object-cover"
              src="../assets/Tassiga/Tassiga-12.png" alt="Banner Image" width="450px">
            <div v-if="showHoverText12"
              class="tw-relative tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-black tw-bg-opacity-20">
              <a href="https://jumbasoft.com/" target="_blank" rel="noopener noreferrer">
                <svg width="50px" version="1.1" id="Uploaded to svgrepo.com" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" xml:space="preserve" fill="#000000">
                  <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                  <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <style type="text/css">
                      .avocado_een {
                        fill: #FD8D14;
                      }

                      .avocado_acht {
                        fill: #FD8D14;
                      }

                      .avocado_zeven {
                        fill: #FFFAEE;
                      }

                      .st0 {
                        fill: #FD8D14;
                      }

                      .st1 {
                        fill: #CC4121;
                      }

                      .st2 {
                        fill: #FFFAEE;
                      }

                      .st3 {
                        fill: #E0A838;
                      }

                      .st4 {
                        fill: #D1712A;
                      }

                      .st5 {
                        fill: #C3CC6A;
                      }

                      .st6 {
                        fill: #6F9B45;
                      }

                      .st7 {
                        fill: #248EBC;
                      }

                      .st8 {
                        fill: #8D3E93;
                      }

                      .st9 {
                        fill: #3D3935;
                      }

                      .st10 {
                        fill: #D36781;
                      }

                      .st11 {
                        fill: #E598A3;
                      }

                      .st12 {
                        fill: #716558;
                      }

                      .st13 {
                        fill: #AF9480;
                      }

                      .st14 {
                        fill: #DBD2C1;
                      }

                      .st15 {
                        fill: #FD8D14;
                      }
                    </style>
                    <g>
                      <path class="avocado_zeven"
                        d="M15.505,9.996c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3 c2.534-2.534,2.534-6.658,0-9.192c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3 C16.181,6.628,15.505,8.261,15.505,9.996z M20.237,8.229c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0 s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3c-0.976,0.976-2.559,0.976-3.536,0S19.261,9.205,20.237,8.229z M0.498,25.004 c0,1.736,0.677,3.369,1.904,4.596c2.535,2.534,6.658,2.534,9.191,0l3.001-3c2.534-2.534,2.534-6.658,0-9.192 c-1.268-1.267-2.932-1.9-4.597-1.9s-3.329,0.633-4.597,1.9l-3,3C1.174,21.635,0.498,23.268,0.498,25.004z M5.23,23.236 c0.636-0.636,2.364-2.364,3-3c0.976-0.976,2.559-0.976,3.536,0s0.976,2.559,0,3.536c-0.636,0.636-2.364,2.364-3,3 c-0.976,0.976-2.559,0.976-3.536,0S4.254,24.212,5.23,23.236z">
                      </path>
                      <g>
                        <path class="avocado_acht"
                          d="M23.5,11c0-0.667-0.26-1.295-0.732-1.768S21.667,8.5,21,8.5s-1.295,0.26-1.768,0.732l-10,10 C8.76,19.705,8.5,20.333,8.5,21s0.26,1.295,0.732,1.768c0.945,0.944,2.59,0.944,3.535,0l10-10C23.24,12.295,23.5,11.667,23.5,11z">
                        </path>
                      </g>
                      <path class="avocado_een"
                        d="M29.95,2.05C28.583,0.683,26.791,0,25,0s-3.583,0.683-4.95,2.05l-3,3 c-1.972,1.972-2.506,4.825-1.632,7.289l-3.074,3.074C11.587,15.144,10.795,15,10,15c-1.791,0-3.583,0.683-4.95,2.05l-3,3 c-2.733,2.734-2.733,7.166,0,9.899C3.417,31.316,5.209,32,7,32c1.791,0,3.583-0.683,4.949-2.05l3-3 c1.972-1.972,2.506-4.825,1.632-7.29l3.074-3.074C20.413,16.856,21.205,17,22,17c1.791,0,3.583-0.683,4.949-2.05l3-3 C32.683,9.216,32.683,4.784,29.95,2.05z M23.985,10.843c-0.037-0.714-0.318-1.419-0.864-1.964c-0.546-0.546-1.25-0.826-1.964-0.864 l2.429-2.429C23.964,5.208,24.466,5,25,5s1.036,0.208,1.414,0.586C26.792,5.964,27,6.466,27,7c0,0.534-0.208,1.036-0.586,1.414 L23.985,10.843z M19.586,9.586C19.964,9.208,20.466,9,21,9s1.036,0.208,1.414,0.586C22.792,9.964,23,10.466,23,11 c0,0.534-0.208,1.036-0.586,1.414l-10,10C12.036,22.792,11.534,23,11,23s-1.036-0.208-1.414-0.586C9.208,22.036,9,21.534,9,21 c0-0.534,0.208-1.036,0.586-1.414L19.586,9.586z M8.015,21.157c0.037,0.714,0.318,1.419,0.864,1.964 c0.546,0.546,1.25,0.826,1.964,0.864l-2.429,2.429C8.036,26.792,7.534,27,7,27s-1.036-0.208-1.414-0.586 C5.208,26.036,5,25.534,5,25c0-0.534,0.208-1.036,0.586-1.414L8.015,21.157z M14.243,26.243l-3,3C10.109,30.376,8.602,31,7,31 c-1.603,0-3.109-0.624-4.243-1.757c-2.339-2.339-2.339-6.146,0-8.485l3-3C6.891,16.624,8.397,16,10,16 c0.53,0,1.046,0.081,1.545,0.213l-6.666,6.666c-1.172,1.171-1.172,3.071,0,4.242C5.465,27.707,6.232,28,7,28 s1.535-0.293,2.121-0.879l6.642-6.642C16.287,22.475,15.804,24.681,14.243,26.243z M29.243,11.243l-3,3 C25.109,15.376,23.602,16,22,16c-0.53,0-1.046-0.081-1.545-0.213l6.666-6.666c1.172-1.171,1.172-3.071,0-4.242 C26.535,4.293,25.768,4,25,4s-1.535,0.293-2.121,0.879l-6.642,6.642c-0.524-1.995-0.041-4.202,1.52-5.763l3-3 C21.891,1.624,23.397,1,25,1s3.109,0.624,4.243,1.757C31.582,5.097,31.582,8.903,29.243,11.243z">
                      </path>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
          </div>

        </div>

      </v-col>

      <!-- View More button -->
      <v-col class="text-center" cols="12">
        <button @click="toggleShowMore"
          class="tw-group bg-hover tw-shadow-lg tw-rounded-lg tw-relative tw-h-12 tw-mt-2 tw-cursor-pointer hover:tw-transform hover:tw-translate-x-2 hover:tw-transition-transform hover:tw-duration-300 tw-shadow-yellow-600 hover:tw-shadow-2xl tw-w-48 tw-overflow-hidden tw-rounded-lg tw-text-lg tw-shadow-2xl">
          <div
            class="tw-absolute tw-inset-0 tw-w-full bg-color tw--transition-all tw-duration-[250ms] tw-ease-out group-hover:tw-w-full">
          </div>
          <span class="tw-relative tw-inline-flex tw-items-center tw-gap-2 tw-text-white group-hover:tw-text-white">
            {{ showMore ? 'View Less' : 'View More' }}
          </span>
        </button>
      </v-col>
    </v-row>
  </div>
</template>
  
<script>
export default {
  data() {
    return {
      showMore: false,
      initialized: false,
      showHoverText: false,
      showHoverText2: false,
      showHoverText3: false,
      showHoverText4: false,
      showHoverText5: false,
      showHoverText6: false,
      showHoverText7: false,
      showHoverText8: false,
      showHoverText9: false,
      showHoverText10: false,
      showHoverText11: false,
      showHoverText12: false,
      showHoverText13: false,
    };
  },
  methods: {
    toggleShowMore() {
      this.showMore = !this.showMore;
    },
  },
};
</script>

<style scoped>
button {
  border: 1px solid #FD8D14;
  background-color: white;

}


.bg-color:hover {
  background-color: #FD8D14;
  color: white;
}

.bg-hover:hover {
  background-color: #FD8D14;
  color: white;
}

.bg-color {
  background-color: #FD8D14;
  color: white;
}

.bg-color-1 {
  background-color: #FD8D14;
}

.bg-image {
  background-image: url('https://img.freepik.com/free-photo/top-view-workspace-with-notebook-succulent-plant_23-2148430820.jpg?w=740&t=st=1690220651~exp=1690221251~hmac=a7249088cdb9fbe455226feccdb5ea68d2bd60b9c7ae35f497131c6e9a06e731');
  background-size: cover;
  height: 100%;
}

.service {
  background-image: url('https://img.freepik.com/free-photo/abstract-luxury-blur-dark-grey-black-gradient-used-as-background-studio-wall-display-your-products-plain-studio-background_1258-54444.jpg');
  background-size: cover;
  height: 100%;
}

.bg-image-1 {
  background-image: url('../assets/omapny.png');
  background-size: cover;
  height: 100%;
}


.text-color {
  color: #FD8D14;
}

/* Popup */
.tw-modal {
  opacity: 0;
  visibility: hidden;
  transition: all 0.3s ease-in-out;
}

.tw-modal-overlay {
  opacity: 0;
  visibility: hidden;
  transition: all 0.3s ease-in-out;
}

.tw-modal-container {
  opacity: 0;
  visibility: hidden;
  transform: translateY(-50px);
  transition: all 0.3s ease-in-out;
}

.tw-modal .tw-modal-overlay {
  opacity: 1;
  visibility: visible;
}

.tw-modal.open {
  opacity: 1;
  visibility: visible;
}


/* Popup */
.tw-modal {
  opacity: 0;
  visibility: hidden;
  transition: all 0.3s ease-in-out;
}

.tw-modal-overlay {
  opacity: 0;
  visibility: hidden;
  transition: all 0.3s ease-in-out;
}

.tw-modal-container {
  opacity: 0;
  visibility: hidden;
  transform: translateY(-50px);
  transition: all 0.3s ease-in-out;
}

.tw-modal .tw-modal-overlay {
  opacity: 1;
  visibility: visible;
}

.tw-modal.open {
  opacity: 1;
  visibility: visible;
}

.tw-modal.open .tw-modal-container {
  transform: translateY(0);
  opacity: 1;
  visibility: visible;
}

/* Close button */
.tw-modal-close {
  cursor: pointer;
}

/* Animation */
@keyframes fadeIn {
  from {
    opacity: 0;
  }

  to {
    opacity: 1;
  }
}

.animated {
  animation-duration: 0.3s;
  animation-fill-mode: both;
}

.fadeIn {
  animation-name: fadeIn;
}
</style>
  